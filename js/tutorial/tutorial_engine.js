function createTutorial(context, interpreter, messager, doc) {
  ...
  function createSection(name, preFun, paragraphs, postFun) {
    // console.log(name, paragraphs);
    // localizer[]
    name = localizer[name] || name;
    for (var i = 0; i < paragraphs.length; i++) {
      paragraphs[i] = localizer[paragraphs[i]] || paragraphs[i];
    }
  ...
  function registerSection(section) {
    tutorial_sections.push(section);
  }
  ...
  function showCommandOneByOne(commands, userInputAccepterCreator) {
    ...
    var commandPostFun =
      commandObj.postFun !== undefined ? commandObj.postFun : G.nothing;
    ...
    messager.listenTo("abort_section", function () {
      // XXX: this will linger if no abort_section is done
...
function runSections(context, interpreter, messager, sections, doc) {
  ...
  function toggleEmphasized() {
    emphasized = !emphasized;
  }

  function getCode(char) {
    return G.charToInt(char) === 32 ? "Space" : char;
  }
  ...
  function writeChar(code) {
    if (code === emphasizedChar) toggleEmphasized();
      ...
      // if not space, let's hack so we can get styling easily
      var $ch = $(doc.getChar(code));

      if (emphasized) $ch.addClass("emphasized");
  ...
  function showParagraph(text, timeout, continuation) {
    if (text.length === 0 || abortCurrentSection) {
      ...
      if (!anykeyPressedDuringSectionShow) {
        ...
        setTimeout(function () {
          showParagraph(rest, timeout, continuation);
        }, chartime);
  ...
  function showText(lines, timeout, continuation) {
    if (lines.length === 0 || abortCurrentSection) {
      ...
      if (!anykeyPressedDuringSectionShow)
        showParagraph(paragraph, charTimeout, function () {
          showRest(rest);
        });
      else
        showParagraph(paragraph, 0, function () {
          showRest(rest);
        });
    }
  }
  ...
  function nextSection() {
    ...
    currentSectionIndex =
      currentSectionIndex !== undefined ? currentSectionIndex + 1 : 0;
    ...
    interpreter.environment.setInsertMode();
    showText(G.reverse(texts), paragraphTimeout, function () {
      afterSectionTexts(currentSection);
    });
  }
  ...
  function bindShowSection() {
    $(".section_menu_item").live("click", function () {
      ...
      setTimeout(function () {
        messager.sendMessage("abort_section");
      }, 0);
    });
  }
  ...
  function showCurrentSectionInMenu() {
    ...
    $(".section_menu_item[number=" + currentSectionIndex + "]").addClass(
      "selected"
    );
  }
